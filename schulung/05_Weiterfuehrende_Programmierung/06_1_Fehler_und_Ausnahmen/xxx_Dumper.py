...
def atomic_type(t):
    return t in ("<type 'none'>", "<type 'str'>", "<type 'int'>", "<type 'long'>", "<type 'float'>", "<type 'complex'>")
...
def simple_value(val):
    ...
    if (not t in DICT_TYPES and t not in ("<type 'list'>", "<type 'tuple'>") and
            not is_instance(val)):
        return True

    elif t in ("<type 'list'>", "<type 'tuple'>") and len(val) <= 10:
        for x in val:
