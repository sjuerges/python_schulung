...
doc = minidom.parse('config_attribute.xml')
...
def allesAusgeben(kindListe, level=0):
    for kind in kindListe:
        ...
        if kind.nodeType == kind.ELEMENT_NODE:
            ...
            ### Attribute holen
            if kind.attributes.keys():
                print(' ' * (level + 1) * 4, 'Attribut-Liste f�r :', kind.tagName)
                for att in kind.attributes.keys():
                    print(' ' * (level + 2) * 4, att, ':', kind.attributes[att].value)
